// Production Loyalty Widget for Shopify Integration
(function() {
  'use strict';
  
  const CONFIG = {
    domain: window.location.origin,
    apiBase: '/functions/v1'
  };
  ...
  function insertWidget(container) {
    try {
      ...
      let widget = document.getElementById('square-loyalty-widget');
      if (!widget) {
        widget = document.createElement('div');
        widget.id = 'square-loyalty-widget';
        widget.className = 'square-loyalty-widget';
      }

      // Insert widget before checkout buttons or at top of container
      let anchor = null;
      ...
      console.log('Square loyalty widget embedded');
      return true;
    } catch (e) {
  ...
  function init() {
    console.log('Square loyalty widget loaded');
  ...
  window.initializeLoyaltyWidget = function() {
    const widget = document.getElementById('square-loyalty-widget');
    if (!widget) return;
    
    widget.innerHTML = `
      <div class="loyalty-widget__header" onclick="toggleLoyalty()">
        <h3 class="loyalty-widget__title">🎁 Loyalty Rewards</h3>
    ...
    // Add styles
    if (!document.getElementById('loyalty-widget-styles')) {
  ...
  window.loadCustomerLoyalty = async function() {
    try {
      showLoading();
      
      const response = await fetch(`${CONFIG.domain}${CONFIG.apiBase}/loyalty-account`, {
        method: 'POST',
  ...
  window.connectLoyalty = async function() {
    ...
    try {
      ...
      const response = await fetch(`${CONFIG.domain}${CONFIG.apiBase}/loyalty-lookup`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (data.success && data.loyalty_account) {
        showLoyaltyAccount(data.loyalty_account, data.rewards || []);
      } else {
  ...
  window.redeemReward = async function(rewardId, accountId) {
    try {
      const response = await fetch(`${CONFIG.domain}${CONFIG.apiBase}/loyalty-redeem`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ loyalty_account_id: accountId, reward_id: rewardId })
      });
  ...
  window.showLoyaltyAccount = function(account, rewards) {
    ...
    if (rewards && rewards.length > 0) {
      rewards.forEach(reward => {
        ...
        rewardDiv.innerHTML = `
          <span class="loyalty-widget__reward-text">${reward.name} - ${reward.points_required} pts</span>
          <button onclick="redeemReward('${reward.id}', '${account.id}')" class="loyalty-widget__redeem-btn">Redeem</button>
        `;
