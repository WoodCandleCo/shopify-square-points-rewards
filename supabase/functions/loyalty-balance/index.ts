...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`Fetching balance for account ${accountId} in ${environment} environment`)

    // Retrieve loyalty account from Square
    const loyaltyResponse = await fetch(`${baseUrl}/v2/loyalty/accounts/${accountId}`, {
      method: 'GET',
      headers: {
    ...
    if (!loyaltyResponse.ok) {
      ...
      console.error('Loyalty account retrieval failed:', errorText)
      
      if (loyaltyResponse.status === 404) {
        return new Response(
